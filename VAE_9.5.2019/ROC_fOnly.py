...
LM='/big_disk/akrami/git_repos/lesion-detector/src/VAE_GANs/figs_VAE'
##########call network##########
...
#G2.eval()
    ...
    with torch.no_grad():
        for i, data in enumerate(Validation_loader_inference):
            ...
            f_recon_batch = arr_lowrec[:, :, :, :]

            

            f_data = data[:, 2, :, :]
            #f_recon_batch = recon_batch[:, 0, :, :]
            rec_error = (f_data - f_recon_batch)**2
            if i<2:
                n = min(f_data.size(0), 100)
                err=(f_data.view(batch_size,3, 128, 128)[:n] -
                     f_recon_batch.view(batch_size,3, 128, 128)[:n])
                err=err**2
                median=(err).to('cpu')
                median=median.numpy()
                median=scipy.signal.medfilt(median,(1,7,7))
                median=median.astype('float32')
                median = np.clip(median, 0, 1)
                scale_error=np.max(median,axis=1)
                scale_error=np.max(scale_error,axis=1)
                scale_error=np.reshape(scale_error,(-1,1,1))
                err=median/scale_error
                err=torch.from_numpy(err)
                err=(err).to(device)

                comparison = torch.cat([
                    f_data[:,2,:,:].view(batch_size, 1, 128, 128)[:n],
                    f_recon_batch[:,0,:,:].view(batch_size, 1, 128, 128)[:n],
