...
X = X/ max_val
X= X.astype('float64')
X_valid,X_train, = train_test_split(X, test_size=0.1, random_state=10002,shuffle=False)
X=X_valid
    ...
    with torch.no_grad():
        for i, data in enumerate(Validation_loader_inference):
            ...
            if i==0:
                ...
                seg_all_all=seg_all
                ind_list=np.array([l1])
            else:
                rec_error_all = torch.cat([rec_error_all, rec_error])
                seg_all_all = torch.cat([seg_all_all, seg_all])
                ind_list=np.concatenate((ind_list,np.array([ind+l1])))
            ind=ind+batch_size
...
if __name__ == "__main__":
    rec_error_all,ind_list,seg_all = Validation(X)
    
    y_true = seg_all.to('cpu')
    y_true = y_true.numpy()
    y_true=y_true[ind_list,:,:,:]
    y_true = np.reshape(y_true, (-1, 1))
    #y_probas = rec_error_all.veiw(-1,1)
    y_probas = (rec_error_all).to('cpu')
    y_probas = y_probas.numpy()
    y_probas=y_probas[ind_list,:,:]
