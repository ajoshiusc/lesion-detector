...
def beta_prob_loss_function(recon_x,var_x, x, mu, logvar,beta):
    
 
    std = var_x.mul(0.5).exp_()
    dim=(128*128*3)
    std_all=torch.prod(std,dim=(1,2,3))
    term1=-(beta+1)/(beta*torch.pow((std_all*(2*math.pi)),(dim*beta/2)))
    term2=torch.sum((((recon_x-x_temp)/std)**2),(1, 2,3))
    term2=torch.exp(-(beta*term2))
    term3=-1/(torch.pow(std_all,beta)*(((beta+1)*((2*math.pi)**beta))**(0.5)))
...
for epoch in range(max_epochs):
    ...
    for data in train_loader:
        ...
        if beta==0:
            ...
            prob_err=beta_prob_loss_function(rec_enc,var_enc, datav, mean, logvar,beta) 
        err_enc = prob_err
        opt_enc.zero_grad()
        err_enc.backward()
        opt_enc.step()
    ...
    with torch.no_grad():
        for data in Validation_loader:
            ...
            if beta==0:
                prob_err=prob_loss_function(valid_enc,valid_var_enc, data, mean, logvar)
            else:
                prob_err=beta_prob_loss_function(valid_enc,valid_var_enc, data, mean, logvar,beta)
