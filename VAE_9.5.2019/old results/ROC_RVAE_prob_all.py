...
X_data=d['data']
X=np.rot90(X_data[:,:,:,0:3], k=1, axes=(1, 2))
X_train=np.concatenate((X_train,X[0:-20*40,:,:,:]),axis=0)
X_valid=np.concatenate((X_valid,X[-20*40:,:,:,:]),axis=0)
...
X_test = np.transpose(X_test[:,::2,::2,:] , (0, 3, 1,2))



X_test_seg=X_data[-20*20:,::2,::2,:]
...
#########################################
epoch=19
LM='/big_disk/akrami/git_repos_new/lesion-detector/VAE_9.5.2019/all_data_RVAE'
        ...
        for i, data in enumerate(Validation_loader):
            ...
            if i<20:
                ...
                comparison = torch.cat([
                    ...
                    (f_data.view(batch_size, 1, 64, 64)[:n]-f_recon_batch.view(batch_size, 1, 64, 64)[:n]),
                    sig_plot.view(batch_size, 1, 64, 64)[:n]*10,
                    err_rec.view(batch_size, 1, 64, 64)[:n],
                    seg.view(batch_size, 1, 64, 64)[:n]
                ])
                save_image(comparison.cpu(),
                           'all_data_RVAE/reconstruction_b' +str(i)+ '.png',
                           nrow=n)
           #############save z values###############
            if i==0:
