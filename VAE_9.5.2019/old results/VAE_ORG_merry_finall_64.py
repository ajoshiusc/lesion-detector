...
def save_model(epoch, encoder, decoder):
    torch.save(decoder.cpu().state_dict(), 'RVAE_final/VAE_GAN_decoder_%d.pth' % epoch)
    torch.save(encoder.cpu().state_dict(),'RVAE_final/VAE_GAN_encoder_%d.pth' % epoch)
    decoder.cuda()
    encoder.cuda()
...
def load_model(epoch, encoder, decoder, D):
    #  restore models
    decoder.load_state_dict(torch.load('.RVAE_final/VAE_GAN_decoder_%d.pth' % epoch))
    decoder.cuda()
    encoder.load_state_dict(torch.load('.RVAE_final/VAE_GAN_encoder_%d.pth' % epoch))
    encoder.cuda()
