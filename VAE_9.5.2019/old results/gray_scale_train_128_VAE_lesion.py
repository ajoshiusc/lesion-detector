...
blb=np.load('blb1.npy')
temp_img=(fixed_batch[3:4,:,:,:]).cpu().numpy()
temp_img[blb>0.1]=temp_img[blb>0.1]
fixed_batch [3,:,:,:]=torch.from_numpy(temp_img)
#blb=lesion_generator()
blb=np.load('blb2.npy')
temp_img=(fixed_batch[2:3,:,:,:]).cpu().numpy()
temp_img[blb>0.1]=blb[blb>0.1]
fixed_batch [2,:,:,:]=torch.from_numpy(temp_img)
...
for epoch in range(max_epochs):
    ...
    for data in train_loader:
        ...
        blb=lesion_generator()
        temp_img=(datav[l1:l1+1,:,:,:]).cpu().numpy()
        temp_img[blb>0.1]=blb[blb>0.1]
