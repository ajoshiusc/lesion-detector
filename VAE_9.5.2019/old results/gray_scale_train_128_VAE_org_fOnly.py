...
def show_and_save(file_name,img):
    f = "/big_disk/akrami/git_repos/lesion-detector/src/VAE_GANs/fig_VAE_fOnly/%s.png" % file_name
...
for epoch in range(max_epochs):
    ...
    for data in train_loader:
        ...
        beta_err=beta_loss_function(rec_enc, datav[:,2:3,:,:], mean, logvar,beta) 
        err_enc = beta_err
        opt_enc.zero_grad()
        err_enc.backward()
        opt_enc.step()
    ...
    with torch.no_grad():
        for data in Validation_loader:
            ...
            beta_err=beta_loss_function(valid_rec, data[:,2:3,:,:], mean, logvar,beta) 
    ...
    show_and_save('rec_epoch_%d.png' % epoch ,make_grid((rec_imgs.data).cpu(),8))
    samples = G.decoder(fixed_noise)
    show_and_save('samples_epoch_%d.png' % epoch ,make_grid((samples.data).cpu(),8))
    show_and_save('Error_epoch_%d.png' % epoch ,make_grid((fixed_batch.data[:,2:3,:,:]-rec_imgs.data).cpu(),8))
