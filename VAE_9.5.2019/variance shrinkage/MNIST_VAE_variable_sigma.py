...
def prob_loss_function(recon_x, var_x, x, mu, logvar):
    ...
    NDim = torch.sum(msk,(1,2))

    std = var_x.mul(0.5).exp_().cuda()

    const = (-torch.sum(var_x.cuda()*msk, (1, 2))) / 2
...
def train(epoch):
    ...
    for batch_idx, (data, data_lab) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        mean, logvar, rec_enc = model(data)
        var_enc=torch.from_numpy(np.zeros((1)))+constsig
