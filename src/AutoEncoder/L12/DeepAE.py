...
if __name__ == "__main__":
    ...
    start_time = time.time()
    with tf.Session() as sess:
        ae = Deep_Autoencoder(sess = sess, input_dim_list=[784,400,200])
        ae.fit(x ,sess = sess, learning_rate=0.01, batch_size = 133, iteration = 100, verbose=True)
        AEL = ae.getRecon(x, sess = sess)
