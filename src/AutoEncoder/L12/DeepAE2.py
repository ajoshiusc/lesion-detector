...
class Deep_Autoencoder(object):
    def __init__(self,input_dim_list=[784,400]):
...
def test():
    ...
    ae = Deep_Autoencoder(input_dim_list=[784,625,400,225,100])
    error = ae.fit(x[:1000] , learning_rate=0.01, batch_size = 500, iteration = 1000, verbose=False)

    print ("size 1000 Runing time:" + str(time.time() - start_time) + " s")

    start_time = time.time()
    ae = Deep_Autoencoder(input_dim_list=[784,625,400,225,100])
    error = ae.fit(x[:10000] , learning_rate=0.01, batch_size = 500, iteration = 1000, verbose=False)

    print ("size 10,000 Runing time:" + str(time.time() - start_time) + " s")

    start_time = time.time()
    ae = Deep_Autoencoder(input_dim_list=[784,625,400,225,100])
    error = ae.fit(x[:20000] , learning_rate=0.01, batch_size = 500, iteration = 1000, verbose=False)

    print ("size 20,000 Runing time:" + str(time.time() - start_time) + " s")

    start_time = time.time()
    ae = Deep_Autoencoder(input_dim_list=[784,625,400,225,100])
    error = ae.fit(x[:50000] , learning_rate=0.01, batch_size = 500, iteration = 1000, verbose=False)
...
if __name__ == "__main__":
    ...
    import os
    #os.chdir("../../")
    x = np.load(r"data.npk")
    start_time = time.time()
    ae = Deep_Autoencoder(input_dim_list=[784,225,100])
    error = ae.fit(x , learning_rate=0.01, batch_size = 500, iteration = 500, verbose=True)
    R = ae.getRecon(x)
    print ("size 100 Runing time:" + str(time.time() - start_time) + " s")
    error = ae.fit(R , learning_rate=0.01, batch_size = 500, iteration = 500, verbose=True)
    #test()