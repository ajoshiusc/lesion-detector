...
if DO_TRAINING:
    ...
    train(model1, data, device='cuda', epochs=50, batch_size=64 * 2)
...
for subid in tbidoneIds:
    t1 = ni.load_img(os.path.join(study_dir, subid, 'T1mni.nii.gz')).get_data()
    t1msk = np.float32(
        ni.load_img(os.path.join(study_dir, subid,
                                 'T1mni.mask.nii.gz')).get_data() > 0)
    t2 = ni.load_img(os.path.join(study_dir, subid, 'T2mni.nii.gz')).get_data()
    flair = ni.load_img(os.path.join(study_dir, subid,
                                     'FLAIRmni.nii.gz')).get_data()
    t1o = ni.load_img(os.path.join(study_dir, subid, 'T1mni.nii.gz'))
    t1msko = ni.load_img(os.path.join(study_dir, subid, 'T1mni.mask.nii.gz'))
    ...
    #%%
#    out_vol = (out_vol).to('cpu')

    t1 = ni.new_img_like(t1o, out_vol[:, :, :, 0] * pt1)
    t1.to_filename(os.path.join(subid + '_T1mni_rec.nii.gz'))

    t1mskfile = ni.new_img_like(t1msko, t1msk)
    t1mskfile.to_filename(os.path.join(subid + '_T1mni_rec.mask.nii.gz'))

    t2 = ni.new_img_like(t1o, out_vol[:, :, :, 1] * pt2)
    t2.to_filename(os.path.join(subid + '_T2mni_rec.nii.gz'))

    flair = ni.new_img_like(t1o, out_vol[:, :, :, 2] * pflair)
    flair.to_filename(os.path.join(subid + '_Flairmni_rec.nii.gz'))

    err = ni.new_img_like(t1o, np.mean((out_vol - dat.numpy())**2, axis=3))
    err.to_filename(os.path.join(subid + '_Error_rec.nii.gz'))

    err = ni.new_img_like(
        t1o,
        np.mean((out_vol[..., 2, None] - dat.numpy()[..., 2, None])**2,
                axis=3))
    err.to_filename(os.path.join(subid + '_Flair_err_rec.nii.gz'))
