...
def auto_encoder(input_size):
    ...
    x = Conv2D(32, (3, 3, 3), activation='relu', padding='same')(input_img)
    x = MaxPooling2D((2, 2, 2), padding='same')(x)
    x = Conv2D(32, (3, 3, 3), activation='relu', padding='same')(x)
    encoded = MaxPooling2D((2, 2, 2), padding='same')(x)

    # at this point the representation is (7, 7, 32)

    x = Conv2D(32, (3, 3, 3), activation='relu', padding='same')(encoded)
    x = UpSampling2D((2, 2, 2))(x)
    x = Conv2D(32, (3, 3, 3), activation='relu', padding='same')(x)
    x = UpSampling2D((2, 2, 2))(x)
    decoded = Conv2D(1, (3, 3, 3), activation='sigmoid', padding='same')(x)
